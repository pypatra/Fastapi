...
def create_user(user: schemas.UserCreate, db:Session = Depends(get_db)) -> models.User:
    db_user: models.User | None = crud.get_user_by_email(db,email=user.email)
    if db_user:
        raise HTTPException(status_code=400, detail="Email Sudah Ada")
    return crud.create_user(db=db, user=user)

@app.get("/users/", response_model= list[schemas.User])
def read_users(skip:int = 0, limit:int = 100, db: Session= Depends(get_db)) -> list[models.User]:
    users: list[models.User] = crud.get_users(db,skip=skip,limit=limit)
    return users
...
def read_items(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)) -> list[models.Item]:
    items: list[models.Item] = crud.get_items(db, skip=skip, limit=limit)
    return items